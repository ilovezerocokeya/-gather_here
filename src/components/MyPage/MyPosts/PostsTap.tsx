...
import { usePostLikeCountStore } from "@/stores/usePostLikeCountStore";
import { getPostLikeCount } from "@/utils/like/getLikeCount";
  ...
  useEffect(() => {
    const fetchAllLikeCounts = async () => {
      for (const post of posts) {
        const count = await getPostLikeCount(post.post_id);
        setLikeCount(post.post_id, count);
      }
    };

    if (posts.length > 0) {
      void fetchAllLikeCounts();
    }
  }, [posts, setLikeCount]);
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-65 text-center z-50">
          <div className="relative min-w-[300px] m:min-w-[260px] p-6 bg-fillStrong rounded-lg shadow-lg z-60">
            <button
              ...
              className="absolute top-3 right-3 text-gray-500 md:hover:text-gray-700"
              aria-label="모달 닫기"
            >
                ...
                {user?.id === post.user_id && (
                  <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity bg-black bg-opacity-50 rounded-2xl">
                    <button
                      ...
                      className="m-2 p-3 bg-fillLight rounded-full md:hover:bg-fillNormal"
                      >
                      <Image
